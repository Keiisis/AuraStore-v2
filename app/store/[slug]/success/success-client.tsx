...
export function SuccessClient({ sessionId, storeName: initialStoreName }: { sessionId?: string, storeName: string }) {
    ...
    // FIX 1: Ref pour garantir que clearCart n'est appelé qu'une seule fois
    // (persiste entre les cycles mount/unmount de StrictMode)
    const cartClearedRef = useRef(false);

    const [loading, setLoading] = useState(true);
    const [order, setOrder] = useState<any>(null);
    // FIX 2: State d'erreur explicite
    const [error, setError] = useState<string | null>(null);

    // FIX 3: fetchOrder stabilisé avec useCallback + support AbortSignal
    const fetchOrder = useCallback(async (signal?: AbortSignal) => {
        ...
        try {
            ...
            while (attempts < 3 && !foundOrder) {
                // FIX 4: Vérifier l'abort AVANT chaque tentative
                if (signal?.aborted) return;

                const result = await getOrderBySessionId(sessionId);

                // FIX 5: Vérifier l'abort APRÈS chaque appel async
                if (signal?.aborted) return;
                ...
                if (result.order) {
                    ...
                    if (attempts < 3) {
                        ...
                        // FIX 6: Vérifier encore après le sleep
                        if (signal?.aborted) return;
                    }
                }
            }

            // FIX 7: setState seulement si pas aborté
            if (!signal?.aborted) {
                if (foundOrder) {
                    setOrder(foundOrder);
                } else {
    ...
    useEffect(() => {
        // FIX 8: AbortController pour cleanup propre en StrictMode
        const abortController = new AbortController();

        // FIX 9: clearCart protégé par un ref — appelé une seule fois
        ...
        // FIX 10: Cleanup — annule le fetch si le composant se démonte
        return () => {
    ...
    // Messages IA — seulement si order existe
    ...
    return (
        <div className="min-h-screen text-black bg-[#F8F9FA] font-sans pt-12 pb-24 px-4 sm:px-8">
            <div className="relative z-10 max-w-[820px] mx-auto space-y-8">
                ...
                {/* ─── OFFICIAL INVOICE — FIX 11: seulement si hasOrder ─── */}
                {hasOrder && (
                    <>
                        <motion.div
                            ...
                            <div className="p-8 sm:p-12 space-y-10">
                                ...
                                <div className="flex items-start gap-4 p-6 bg-gray-50 rounded-2xl border border-gray-100">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-[11px] font-black uppercase tracking-widest text-[#FE7501]">Message de l'Intelligence Aura</p>
