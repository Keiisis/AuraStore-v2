        ...
        try {
            ...
            if (!signal?.aborted) {
                if (foundOrder) {
                    ...
                    setError("Commande introuvable après plusieurs tentatives.");
                }
                setLoading(false);
            }
        } catch (e) {
        ...
        <div className="min-h-screen text-black bg-[#F8F9FA] font-sans pt-12 pb-24 px-4 sm:px-8 print:bg-white print:pt-0 print:pb-0 print:px-0 print:min-h-0">
            <div className="relative z-10 max-w-[820px] mx-auto space-y-8 print:max-w-none print:mx-0 print:space-y-0">
                ...
                <motion.div
                    ...
                    className="flex flex-col items-center gap-4 mb-8 print:hidden"
                >
                    ...
                    <div className="text-center">
                        <h1 className="text-2xl font-black uppercase tracking-tight italic">
                            {hasOrder ? "Commande Confirmée" : "Facture non trouvée"}
                        </h1>
                ...
                {hasOrder && (
                    <>
                        <motion.div
                            ...
                            className="bg-white rounded-[2rem] shadow-[0_10px_50px_rgba(0,0,0,0.08)] border border-gray-100 overflow-hidden print:shadow-none print:rounded-none print:border-0 print:overflow-visible print:fixed print:top-0 print:left-0 print:w-full print:h-auto print:z-[9999]"
                        >
                            {/* Invoice Header */}
                            <div className="p-8 sm:p-12 space-y-10">
                                <div className="flex flex-col sm:flex-row justify-between items-start gap-8 border-b-2 border-black pb-8 print:border-black">
                                    <div>
                                        <h2 className="text-2xl font-black uppercase tracking-widest leading-none print:text-black">{storeName}</h2>
                                        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-[0.25em] mt-2 print:text-black">Facture Officielle Aura</p>
                                        <div className="mt-4 space-y-1 text-[11px] text-gray-500 font-medium print:text-black">
                                            {store.address && <p className="flex items-center gap-2"><MapPin className="w-3.5 h-3.5 opacity-40 print:opacity-100" /> {store.address}</p>}
                                            {store.phone && <p className="flex items-center gap-2"><Phone className="w-3.5 h-3.5 opacity-40 print:opacity-100" /> {store.phone}</p>}
                                        </div>
                                    </div>
                                    <div className="sm:text-right">
                                        <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest mb-1 print:text-black">DÉTAILS FACTURE</p>
                                        <p className="text-xl font-mono font-black print:text-black">{invoiceNumber}</p>
                                        <p className="text-[11px] text-gray-500 mt-1 uppercase font-bold print:text-black">{orderDate}</p>
                                ...
                                <div className="flex items-start gap-4 p-6 bg-gray-50 rounded-2xl border border-gray-100 print:bg-white print:border print:border-gray-300 print:rounded-lg">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-[11px] font-black uppercase tracking-widest text-[#FE7501] print:text-black">Message de l'Intelligence Aura</p>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-12">
                                    <div>
                                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-4 print:text-black">FACTURE À</p>
                                        <div className="space-y-1.5">
                                            <p className="text-base font-bold text-gray-900 print:text-black">{customerName}</p>
                                            {customerEmail && <p className="text-xs text-gray-500 font-medium print:text-black">{customerEmail}</p>}
                                            {customerPhone && <p className="text-xs text-gray-500 font-medium print:text-black">{customerPhone}</p>}
                                            {shippingAddress && <p className="text-xs text-gray-500 font-medium italic mt-2 flex items-start gap-2 print:text-black"><MapPin className="w-3.5 h-3.5 mt-0.5 opacity-30 shrink-0 print:opacity-100" /> {shippingAddress}</p>}
                                        </div>
                                    </div>
                                    <div className="sm:text-right">
                                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-4 print:text-black">PAIEMENT & STATUT</p>
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2 sm:justify-end">
                                                <CreditCard className="w-4 h-4 text-gray-400 print:text-black" />
                                                <span className="text-sm font-bold text-gray-800 capitalize print:text-black">{paymentMethod === 'stripe' ? 'Carte Bancaire (Stripe)' : paymentMethod}</span>
                                            </div>
                                            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-emerald-100 print:bg-white print:text-black print:border-black">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="divide-y divide-gray-100 px-2 print:divide-gray-300">
                                        {items.map((item: any, i: number) => (
                                            <div key={i} className="grid grid-cols-12 gap-4 py-6 items-center">
                                                <div className="col-span-8 flex items-center gap-4">
                                                    <div className="w-14 h-14 bg-gray-50 rounded-xl border border-gray-100 overflow-hidden shrink-0 print:bg-white print:border-gray-300 print:rounded-md">
                                                        {item.image ? (
                                                            <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                                        ) : (
                                                            <div className="w-full h-full flex items-center justify-center"><ShoppingBag className="w-5 h-5 text-gray-200 print:text-gray-500" /></div>
                                                        )}
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-bold text-gray-900 print:text-black">{item.name || "Article"}</p>
                                                        <p className="text-[10px] text-gray-400 mt-1 uppercase font-bold tracking-tight print:text-black">
                                                            {formatPrice(Number(item.price) || 0, "XOF")} / u
                                                            {item.variant ? ` • ${item.variant}` : ""}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div className="col-span-1 text-center font-black text-xs text-gray-500 print:text-black">{item.quantity}</div>
                                                <div className="col-span-3 text-right font-mono font-black text-sm text-gray-900 print:text-black">
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-end gap-10 pt-8 border-t border-gray-100 print:border-gray-300">
                                    <div className="space-y-4 max-w-xs">
                                        <p className="text-[10px] font-bold text-gray-400 leading-relaxed italic uppercase tracking-tighter print:text-black">
                                            Note: {deliveryNote}
                                        </p>
                                    </div>
                                    <div className="w-full sm:w-64 space-y-3">
                                        <div className="flex justify-between items-center text-xs px-2">
                                            <span className="text-gray-400 font-bold uppercase tracking-widest text-[9px] print:text-black">Sous-total</span>
                                            <span className="font-mono font-bold text-gray-600 print:text-black">{formatPrice(subtotal, "XOF")}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-xs px-2">
                                            <span className="text-gray-400 font-bold uppercase tracking-widest text-[9px] print:text-black">Expédition</span>
                                            <span className="text-emerald-600 font-bold tracking-widest text-[9px] uppercase print:text-black">Offerte</span>
                                        </div>
                                        <div className="pt-3 border-t-2 border-dashed border-gray-100 print:border-gray-400">
                                            <div className="flex justify-between items-center p-5 bg-[#FE7501]/5 rounded-2xl border border-[#FE7501]/10 print:bg-white print:border-2 print:border-black print:rounded-lg">
                                                <span className="text-[10px] font-black uppercase tracking-widest text-[#FE7501] print:text-black">TOTAL TTC</span>
                                                <span className="text-2xl font-mono font-black text-[#FE7501] print:text-black">{formatPrice(total, "XOF")}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="px-8 sm:px-12 py-8 bg-gray-50/80 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-6 print:bg-white print:border-gray-300">
                                ...
                                <div className="text-right">
                                    <p className="text-[8px] text-gray-300 font-black uppercase tracking-[0.3em] italic print:text-black">Document généré automatiquement le {orderDate}</p>
                                </div>
                            </div>
                        </motion.div>
